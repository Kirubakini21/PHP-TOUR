...
if ($_FILES["txtFile"]["error"] > 0)
 ...
 //   echo "Upload: " . $_FILES["fpFilePath"]["name"] . "<br />";
    ...
    if (file_exists("image/syllabus/" . $_FILES["txtFile"]["name"]))
      {
      ...
      "image/syllabus/" . $_FILES["txtFile"]["name"]);
	...
	$qry="Insert Into SyllabusTable Values('". $Id ."','". $Date ."','". $Dept .  "','" . $Sem . "','". $_SESSION['username'] ."','". $Info ."','". $Fpath ."')";
	mysql_query($qry,$con) or die(mysql_error());

   	header('location:ViewSyllabusStaff.php');
