...
if ($_FILES["txtFile"]["error"] > 0)
 ...
 //   echo "Upload: " . $_FILES["fpFilePath"]["name"] . "<br />";
    ...
    if (file_exists("image/material/" . $_FILES["txtFile"]["name"]))
      {
      ...
      "image/material/" . $_FILES["txtFile"]["name"]);
	...
	$qry="Insert Into MaterialTable Values('". $Id ."','". $Date ."','". $Dept .  "','" . $Sem . "','". $_SESSION['username'] ."','". $Info ."','". $Fpath ."')";
	mysql_query($qry,$con) or die(mysql_error());

   	header('location:ViewMaterialStaff.php');
