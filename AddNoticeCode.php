...
if ($_FILES["txtFile"]["error"] > 0)
 ...
 //   echo "Upload: " . $_FILES["fpFilePath"]["name"] . "<br />";
    ...
    if (file_exists("image/uploads/" . $_FILES["txtFile"]["name"]))
      {
      ...
      "image/uploads/" . $_FILES["txtFile"]["name"]);
	...
	$qry="Insert Into NoticeTable Values('". $Id ."','". $Date ."','". $Dept ."','". $_SESSION['username'] ."','". $Info ."','". $Fpath ."')";
	mysql_query($qry,$con) or die(mysql_error());

   	header('location:ViewNoticeStaff.php');
